...
% INPUTS
%
% nodes is an nv x n matrix with the coordinates of nodes
%     nodes(i,:) is the location of node i
%     nv is the number of nodes
%     n is the number of spatial dimensions (usually 2 or 3)
% 
...
%     If all Hooke constants densities are the same, k may be a scalar
%     (default 1)
%
...
for i=1:ne
    ...
    % Other columns are any orthonormal basis for the rest of R^n
    [TV(:,:,i),~] = svd(vs(:,i));
end
...
% First n*F rows are Dirichlet conditions on the frame
% Next n*(2*ne-F-nv+1) rows are node continuity conditions
% Last n*(nv-1) rows are force balancing conditions
...
for u=2:nv
    ...
    for i=Nu(2:end)
        ...
        M(row0:row0+n-1,col0i1:col0i1+2*n-1) = negxi1;
        M(row0:row0+n-1,col0i :col0i +2*n-1) = xi;
        row0 = row0 + n;
    end
end
...
% but, for each node
%     only n rows are used (representing sum of all forces in each of n dimensions)
...
for u=2:nv
    ...
    for i=Nu(1:end)
        ...
        M(row0:row0+n-1,col0i :col0i +2*n-1) = -w*Hxip;
    end
    row0 = row0 + n;
end
end % build_mat function

function ci = coeff_index(ab,d,i)
